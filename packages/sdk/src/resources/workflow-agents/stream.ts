...
import { APIError, OpenAIError } from '../../core/error';
import { Stream, _iterSSEMessages } from '../../core/streaming';
    ...
    async function* iterator(): AsyncGenerator<Item> {
      ...
      try {
        ...
        if (!(error instanceof APIError) &&
          (isAbortError(error) || (controller.signal.aborted && error === controller.signal.reason))) return;
        throw error;
      } finally {
