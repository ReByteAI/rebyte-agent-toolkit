// Modified by Rebyte; see UPSTREAM.md for fork changes and provenance.
...
export interface ClientOptions {
   ...
   * - Defaults to process.env['REBYTE_API_KEY'].
  ...
  /**
   * Defaults to process.env['REBYTE_ADMIN_KEY'].
   */
  adminAPIKey?: string | null | undefined;

  /**
   * Defaults to process.env['REBYTE_ORG_ID'].
   */
  organization?: string | null | undefined;

  /**
   * Defaults to process.env['REBYTE_PROJECT_ID'].
   */
  project?: string | null | undefined;

  /**
   * Defaults to process.env['REBYTE_WEBHOOK_SECRET'].
   */
   ...
   *
   * Defaults to process.env['REBYTE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['REBYTE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API client for Rebyte, forked from the OpenAI TypeScript SDK.
 */
export class OpenAI {
  apiKey: string | null;
   ...
   * API client for Rebyte, forked from the OpenAI TypeScript SDK.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['REBYTE_API_KEY'] ?? null]
   * @param {string | null | undefined} [opts.adminAPIKey=process.env['REBYTE_ADMIN_KEY'] ?? null]
   * @param {string | null | undefined} [opts.organization=process.env['REBYTE_ORG_ID'] ?? null]
   * @param {string | null | undefined} [opts.project=process.env['REBYTE_PROJECT_ID'] ?? null]
   * @param {string | null | undefined} [opts.webhookSecret=process.env['REBYTE_WEBHOOK_SECRET'] ?? null]
   * @param {string} [opts.baseURL=process.env['REBYTE_BASE_URL'] ?? https://api.rebyte.ai/v1] - Override the default base URL for the API.
  ...
  constructor(clientOptions: ClientOptions = {}) {
    // Rebyte fork: never select an OpenAI/provider endpoint implicitly.
    if (clientOptions.dataResidency != null || clientOptions.provider != null ||
        clientOptions.workloadIdentity != null || clientOptions.credential != null ||
        clientOptions.x509Transport != null) {
      throw new Errors.OpenAIError('Rebyte supports API-key authentication. Provider, dataResidency and workload-identity options are not supported.');
    }
    ...
    const {
      baseURL = provider ? null : readEnv('REBYTE_BASE_URL'),
      dataResidency: _dataResidency,
      [inheritedDataResidencySelection]: inheritedResidencySelection = false,
      apiKey = provider ? null : (readEnv('REBYTE_API_KEY') ?? null),
      adminAPIKey = provider ? null : (readEnv('REBYTE_ADMIN_KEY') ?? null),
      organization = provider ? null : (readEnv('REBYTE_ORG_ID') ?? null),
      project = provider ? null : (readEnv('REBYTE_PROJECT_ID') ?? null),
      webhookSecret = readEnv('REBYTE_WEBHOOK_SECRET') ?? null,
      workloadIdentity,
      x509Transport,
    ...
    const options: ClientOptions = {
      ...
      baseURL:
        ...
        residencyBaseURL ??
        (baseURL || (usesX509Identity ? X509_API_BASE_URL : `https://api.rebyte.ai/v1`)),
    };
    ...
    if (!providerRuntime && !apiKey && !adminAPIKey && !workloadIdentity) {
      throw new Errors.OpenAIError(
        'Missing credentials. Please pass an `apiKey`, `workloadIdentity`, `adminAPIKey`, or set the `REBYTE_API_KEY` or `REBYTE_ADMIN_KEY` environment variable.',
      );
    }
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('REBYTE_LOG'), "process.env['REBYTE_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = provider || credential ? undefined : readEnv('REBYTE_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return (
      ...
      this.baseURL !== 'https://api.rebyte.ai/v1'
    );
  }
